###############################################################
###############################################################
-- Guided Project: SQL CASE Statements  - Postgresql
###############################################################
###############################################################
...
CASE
	WHEN EXTRACT(YEAR FROM hire_date) < '1990' THEN 'Employed before 1999'
...
SELECT e.emp_no, e.first_name, e.last_name, s.salary, MAX(s.salary) - MIN(s.salary) AS Salary_difference,
CASE
	WHEN MAX(s.salary) - MIN(s.salary) > 30000 THEN 'Salary was raised by more than $30,000'
	WHEN MAX(s.salary) - MIN(s.salary) BETWEEN 200000 AND 30000 THEN 'Salary was raised by more than $20,000 but less than $30,000'
...
SELECT SUM(CASE WHEN profit < 0 THEN 1 ELSE 0) AS no_profit,
SUM(CASE WHEN profit > 0 AND profit < 500 THEN 1 ELSE 0) AS low_profit,
SUM(CASE WHEN profit > 500 AND profit < 1500  THEN 1 ELSE 0) AS good_profit,
